...
export default async function WorkflowDetailsPage(props: {
  ...
  // Fetch person ID for the current authenticated user
  let personId = '';
  if (orgId && authOrg?.userId) {
    const { data: personData } = await supabaseAdmin
      .from('persons')
      .select('id')
      .eq('auth_user_id', authOrg.userId)
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
      ...
      <div style={{ display: 'flex', gap: '24px', alignItems: 'flex-start' }}>
        ...
        {/* Kanban Board */}
        <div style={{ flex: 1, minWidth: 0, display: 'flex', gap: '20px', overflowX: 'auto', paddingBottom: '16px' }}>
