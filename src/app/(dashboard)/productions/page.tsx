...
export default async function ProductionsPage() {
  ...
  if (error) {
    return <div>Error loading productions.</div>;
  }
  ...
  return (
    <div>
      <header className="q-page-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
        <div>
          <h1 className="q-page-title">Productions</h1>
      ...
      <div className="q-card">
        {(!productions || productions.length === 0) ? (
          <div style={{ textAlign: 'center', padding: '48px', color: 'var(--q-color-ink-500)' }}>
            No active productions. Create a manual booking or wait for a client to book via your storefront.
          </div>
        ) : (
              ...
              {productions.map((prod: any) => (
                <tr key={prod.id} style={{ borderBottom: '1px solid var(--q-color-ink-100)' }}>
                  ...
                  <td style={{ padding: '16px', textAlign: 'right' }}>
                    <Link href={`/productions/${prod.id}`} className="q-btn q-btn-secondary" style={{ fontSize: '0.875rem' }}>
                      Open Workspace
                    </Link>
                  </td>
                </tr>
